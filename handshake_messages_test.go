...
func TestMarshalUnmarshal(t *testing.T) {
	...
	for i, iface := range tests {
		...
		for j := 0; j < n; j++ {
			...
			if ch, ok := m2.(*clientHelloMsg); ok {
				// extensions is special cased, as it is only populated by the
				// server-side of a handshake, and this not expected to roundtrip
				// through marshal + unmarshal.  m2 ends up with the list of
				// extensions necessary to serialize the other fields of clientHelloMsg,
				// so check that it is non-empty, then clear it.
